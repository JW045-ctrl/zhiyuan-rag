# 知源 Zhiyuan：零基础也能复刻的 Dify + Next.js RAG 知识助手

> 从 PDF 学习资料到结构化项目文档，从流式回答、真实引用到固定评估——完整记录一个 RAG 项目如何用证据检查答案。

## 项目开场

很多 RAG 教程停在“上传 PDF 后能够回答问题”这一步，但真正的问题是：它找对资料了吗？引用真的支持答案吗？资料不足时会不会胡编？

知源从一个大一微积分学习助手开始，完整实现了知识库问答、流式输出、真实引用、无资料拒答和固定评估集，并诚实记录了复杂 PDF 中公式、跨页内容和相邻例题带来的检索问题。v0.2进一步加入虚构的“悦享咖啡会员小程序2.0升级项目”知识包，验证同一套机制如何服务项目范围、里程碑、会议决定、风险和验收标准查询。

这里的 **RAG（Retrieval-Augmented Generation，检索增强生成）**，可以简单理解为：AI回答前先从指定资料中找依据，再根据找到的内容组织答案。知源不是完整课程平台或成熟企业产品，而是一个可以复现、检查和继续学习的原型项目。

## 30秒了解知源

| 你可能关心的问题 | 当前项目的回答 |
| --- | --- |
| 它现在能做什么？ | 围绕指定资料回答问题，逐步显示答案，并给出本次真实检索片段。 |
| 两个验证场景是什么？ | v0.1用大一微积分测试复杂PDF；v0.2用虚构项目交付资料测试多文档事实、时间状态和拒答边界。 |
| 只能处理微积分吗？ | 不是。v0.2已经接入9份短、结构化Markdown组成的企业项目演示知识包。 |
| 回答可靠吗？ | v0.2固定15题中13题严格通过，两个失败和适用边界均完整保留。 |
...
这套框架并不限定学科。v0.2已经加入一个完全虚构的**单项目交付知识包**，用9份Markdown验证同一套问答、引用和拒答机制在非课程场景中的表现。知识包事实、实验过程和最终冻结配置见 [v0.2项目交付知识包](knowledge-packs/project-delivery-demo/README.md)。
...
- 项目知识助手：<http://127.0.0.1:3000/assistant>
...
仓库保存了 Dify 1.16.1 导出的v0.1 Chatflow DSL：
...
知源v0.1配置继续保持冻结。v0.2使用独立知识包、独立测试集和独立Dify知识库；其最终配置与实验依据见 [v0.2项目交付知识包](knowledge-packs/project-delivery-demo/README.md)。两组配置都只代表对应资料和测试集，不应直接复制为其他知识库的最佳参数。

当前仓库公开v0.2知识包和测试集，但不包含已建好的Dify知识库、索引、本地数据库或应用Key。新环境仍需创建知识库、上传9份正式Markdown、绑定知识检索节点并发布自己的应用；不能只靠仓库一键恢复本地Dify状态。
...
knowledge-packs/           v0.2虚构项目知识包、正式资料与独立测试集
scripts/                   RAG 批量评估脚本
...
v0.2使用9份已审核的短Markdown和独立固定测试集。经过Baseline、Markdown H2父段诊断和全文父上下文实验后，最终采用“子Chunk检索、返回全文父上下文”的exp02配置。
...
Baseline为10/15，exp02恢复4题、回归1题，净收益为+3。剩余的稳定RAG限制是 `project-answer-006` 所需scope文档没有进入Chatflow最终上下文；另一次正式运行观察到 `2032 → 2022` 的孤立年份转写错误，但目前没有证据表明它是稳定可复现的配置缺陷。

5道REFUSE题人工复核全部通过，自动检测器却显示0/5，因为其固定短语没有覆盖“未提供”“未定义”“没有具体日期”和“待确认”等合理拒答表达。这是评估工具问题，不是实际拒答能力为0/5。详细实验链、冻结配置和适用边界见 [v0.2项目交付知识包说明](knowledge-packs/project-delivery-demo/README.md)。
...
6. **短Markdown和复杂PDF需要不同策略。** v0.2的全文父上下文改善了短文档证据完整性，但这个结论不能泛化到长文档或大型企业知识库。
...
- v0.2全文父上下文只在9份短、结构化Markdown上验证，不代表适合任意企业PDF或长文档；
- v0.2仍有一题因多文档证据没有全部进入最终上下文而无法完整回答。
...
- 为未来长文档评估按业务章节建立语义父块和结构预处理；
- 把复杂PDF标题、表格、图片与跨页内容处理作为独立流程评估；
